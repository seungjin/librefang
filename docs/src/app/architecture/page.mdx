...
| **librefang-channels** | Channel bridge layer with the in-process adapter set + the sidecar mechanism. Each in-process adapter implements the `ChannelAdapter` trait. Includes: WhatsApp, Signal, Matrix, Email, Webhook, Sidecar, Teams, Mattermost, Google Chat, Zulip, LINE, Feishu, DingTalk, QQ, WeChat, WeCom (WeChat Work). Telegram / ntfy / Gotify / Mastodon / Bluesky / Reddit / Twitch / Rocket.Chat / Nextcloud Talk / Discord / Slack / Webex ship as Python sidecars via the `librefang-sdk` package. Features: `AgentRouter` for message routing, `BridgeManager` for lifecycle coordination, `ChannelRateLimiter` (per-user DashMap tracking), `formatter.rs` (Markdown to TelegramHTML/SlackMrkdwn/PlainText), `ChannelOverrides` (model/system_prompt/dm_policy/group_policy/rate_limit/threading/output_format), DM/group policy enforcement, sidecar_channels mechanism for auxiliary channel wiring. |
...
> The Sidecar adapter enables external processes written in any language to act as channel adapters via JSON-RPC over stdin/stdout. Telegram / ntfy / Gotify / Mastodon / Bluesky / Reddit / Twitch / Rocket.Chat / Nextcloud Talk / Discord / Slack / Webex ship as first-party Python sidecars in `librefang-sdk`.
