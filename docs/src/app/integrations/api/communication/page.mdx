...
List configured channel adapters and their status. Supports the in-process adapter set (Discord, Slack, WhatsApp, Matrix, Email, Teams, Mattermost, IRC, Google Chat, Twitch, Rocket.Chat, Zulip, XMPP, LINE, Reddit, and more) plus sidecar channels (Telegram, ntfy, Gotify, Mastodon, Bluesky ship as Python sidecars in `librefang-sdk`).
