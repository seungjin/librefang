...
The native Desktop package is standalone and does not require the CLI package.
First complete the [official Arch repository setup](/getting-started#arch-linux-repository-setup), then install only the Desktop app:

```bash
sudo pacman -Syu librefang-desktop-bin
```

The Desktop package is available on x86_64 only.
...
When the desktop app can't reach a daemon (first launch, or the bundled daemon failed to come up), it now renders a **dedicated connection screen** instead of a white error page. The screen offers:

- "Try again" — re-attempt the connection.
- "Open in browser" — fall back to the daemon's web UI when the embedded WebView is the problem.
- "Uninstall" — full removal from inside the app, useful when the daemon is too broken to reach via the menu bar.

The screen ships as an embedded asset and is rendered via a custom URI scheme (`librefang://connection`) rather than over the daemon's own HTTP — so it works even when the daemon is stopped or the IPC bridge is down. Same path is used on Windows / Linux / macOS.
...
Opens a native file picker for `.toml` files. Validates the selected file as an `AgentManifest`, copies it to `~/.librefang/workspaces/agents/{name}/agent.toml`, and spawns the agent. Returns the agent name on success.
...
The desktop app is the server side of the mobile pairing flow. For the iOS / Android thin-client dashboard that connects to this daemon remotely, see the [Mobile](/integrations/mobile) page.
