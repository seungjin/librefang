...
# WHATSAPP_DM_POLICY = "respond"     # 仅 Cloud API webhook 模式生效
# WHATSAPP_GROUP_POLICY = "all"      # 会被读取但目前不生效
```
...
sidecar 读取与原 Rust 适配器相同的 DM × group 策略环境变量，但有两点注意。
`WHATSAPP_DM_POLICY` 仅在 Cloud API webhook 模式下生效 —— Web/QR 模式下 gateway 直接把入站 POST 给 REST API，绕过该过滤（见上文）。
`WHATSAPP_GROUP_POLICY` 会被读取但目前两种模式下都不生效：Cloud API webhook 把所有入站都当 DM 处理，群组分支永远走不到。

| `WHATSAPP_DM_POLICY` | 行为（Cloud API webhook 模式） |
|---|---|
| `respond`（默认） | 回复所有 DM |
| `allowed_only` | 仅当发送方手机号在 `WHATSAPP_ALLOWED_USERS` 时回复 |
| `ignore` | 丢弃所有 DM |

| `WHATSAPP_GROUP_POLICY` | 文档值（目前不生效） |
|---|---|
| `all`（默认） | 回复所有群消息 |
